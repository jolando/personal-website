...
@Directive({
  selector: '[appMomentum]',
})
export class MomentumDirective implements OnInit {
  private data = {
    ease: 0.07,
    curr: 0,
    prev: 0,
  ...
  private initScrollStyles(): void {
    ...
    if (this.elementInitialized) {
      const appRootStyleObj: CSSStyleDeclaration = (
        this.el.nativeElement.parentNode as HTMLElement
      ).style;
